...
import { z } from 'zod';
import { locales, OTHER_LOCALE, US_REMOTE_LOCALE, type Locale, type WhatDisclosure, Situation, allLocales, type AbstractLocale } from '$lib/data';
...
export function findMatchingLaws(params: MatchParameters, useLocales?: Record<string, Locale>, useAllLocales?: Record<string, AbstractLocale>): Match[] {
    ...
    if (isValidParams(params)) {
        ...
        let availableLocales = useLocales ?? locales;
        let availableAllLocales = useAllLocales ?? allLocales;

        // Company locales are the actual location of the company plus any locales that contain that locale.
        const companyLocales = params.companyLocation !== OTHER_LOCALE ? Object.values(availableLocales).filter((l) =>
            l.isOrContains(availableLocales[params.companyLocation])
        ) : [];
        // Same rubric for users.
        const userLocales = params.userLocation !== OTHER_LOCALE ? Object.values(availableLocales).filter((l) =>
        ...
        for (const thisLocale of Object.values(availableLocales)) {
            ...
            const situationMatch =
                ...
                (params.situation !== Situation.Employed &&
                    params.situation >= disclosureSituations[0]);
